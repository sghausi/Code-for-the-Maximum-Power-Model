%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                        %
% To Implement the maximum power approach described in Ghausi et a (2023)%
...
%load the global land mask generated using IGBP land-cover classification
  ...
  for s = 1:18967
         lon_index = find(grid_land(s,1) == lon_g2);
    lat_index = find(grid_land(s,2) == lat_globe);

    % grid_land is the matrix for land-mask obtained using IGBP land cover classification

      lat_index = grid_land(s,2) - lat_globe(1,1) + 1;
      %%
      % 
      %   for x = 1:10
      %       disp(x)
      %   end
      % 
      lon_index = grid_land(s,1) - lon_globe(1,1) + 1;
      if lon_index < 180.01
          lon_index = lon_index + 180;
      else
          lon_index = lon_index - 180;
      end
...
for s = setdiff(1:18967,del4) %where del4 are the grids on small islands and oceans
      for i = 1:228 % Running for each month from 2001 to 2019
...
if Rin{s,1}(i,1)  - globe_lwtoa_clr{s,1}(i,1) > 3  % cases that does not undergo inversion 

jt = (1:(Rin{s,1}(i,1)  - globe_lwtoa_clr{s,1}(i,1)))';
